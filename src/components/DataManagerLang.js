...
import { Table, Form, Button, notification, Modal,Checkbox } from "antd";
...
const DataManagerIntern = ({ object, callbackitemContent }) => {
  ...
  const fetch = (params = {}) => {
    if (datamember.member_id !== undefined) {
      setLoading(true);
      get(FETCH_URL_LANG + datamember.member_id).then((data) => {
        if (data) {
          setData(data);
          setPagination({
            ...pagination,
  ...
  const columns = [...STATIC_COLUMNS_LANG, ...action_culum];
  useEffect(() => {
    fetch(pagination);
  }, []);
