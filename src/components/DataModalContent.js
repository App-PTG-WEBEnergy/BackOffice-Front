...
import EditBasicContent from "views/pages/contents/EditBasicContent";

import { post, get } from "services/callAPI";
import { menuid } from "views/pages/Menu/datamenuid";

const DataModalContent = ({ object, callbackitemContent }) => {
  const {
    EDITOR,
    ADDEDITOR,
    UPDATE_STATUS_URL,
  ...
  let [visibleBasic, setVisibleBasic] = useState(false);
  ...
  const fetch = (params = {}) => {
    if (updateId.menu_id !== undefined) {
      ...
      get(FETCH_URL + updateId.menu_id).then((data) => {
        if (data[DATA_FETCH]) {
          data[DATA_FETCH].sort((a, b) => (a.sequence > b.sequence ? 1 : -1));
  ...
  const editContent = (edit_data) => {
    setDone(false);
    setType("edit");
    if (edit_data.menu_id == 184 || edit_data.menu_id == 185 || edit_data.menu_id == 287) {
  ...
  const STATIC_COLUMNS_MODIFY =
    data.filter((x) => x.topic_th !== null).length > 0
      ? STATIC_COLUMNS
      : STATIC_COLUMNS_M;
  const columns = [
    ...FixIndexColunm,
    ...STATIC_COLUMNS_MODIFY,
    ...action_culum,
  ];
  const deleteData = () => {
    Modal.confirm({
  ...
  const addData = () => {
    ...
    setVisible(false);
    if (updateId.menu_id === 184 || updateId.menu_id === 185) {
      setVisibleBasic(true);
    } else {
      setVisible(true);
    }
...
export default DataModalContent;
