...
import EditContentTruck from "views/pages/contents/EditContentTruck";

const DataManagerTruck = ({ object, callbackitemContent }) => {
  const {
    ADDEDITOR,
    UPDATE_STATUS_URL,
    INIT_ORDER,
      ...
      get(FETCH_URL + updateId.menu_id).then((data) => {
        if (data[DATA_FETCH]) {
          ...
          data[DATA_FETCH].forEach((element) => {
            var jData = JSON.parse(element.description_th);
...
export default DataManagerTruck;
