...
import EditContentMenuProduct from "views/pages/contents/EditContentProduct";

const DataManagerProduct = ({ object, callbackitemContent }) => {
  const {
    ADDEDITOR,
    UPDATE_STATUS_URL,
    INIT_ORDER,
  ...
  const STATIC_COLUMNS_MODIFY =
    data.filter(x => x.topic_th !== null).length > 0  ? STATIC_COLUMNS : STATIC_COLUMNS_M;
  const columns = [...FixIndexColunm,...STATIC_COLUMNS_MODIFY, ...action_culum];
  const deleteData = () => {
    Modal.confirm({
...
export default DataManagerProduct;
