...
const DataManagerHistory = ({ object, callbackitemContent }) => {
  ...
  const fetch = (params = {}) => {
    if (datamember.member_id !== undefined) {
      setLoading(true);
      get(FETCH_URL_ATTACHMENT + datamember.member_id).then((data) => {
        if (data) {
          setData(data);
          setPagination({
            ...pagination,
  ...
  const columns = [...STATIC_COLUMNS_ATTACHMENT, ...action_culum];
  useEffect(() => {
    fetch(pagination);
  }, []);
            ...
            {data.map((data, index) => {
              return (
                <tr>
                  ...
                  <td>
                    <a target='_blank' href={data.file_path}>{data.file_name}</a>
                  </td>
                </tr>
              );
            })}
